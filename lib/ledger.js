...
export async function getAddress(path = "44'/111111'/0'/0/0", display = false) {
    ...
    const publicKey = Buffer.from(address.subarray(1, 66));
    // Index 66 is always 0x20 = (32) the length of the following chain code
    const chainCode = Buffer.from(address.subarray(67, 67 + 32));
    const yCoordParity = address[65] % 2; // x-coord is index [2, 34), y-coord is index [34, 66)

    return {
        address: publicKeyToAddress(Buffer.from(address.subarray(2, 34)), false),
        publicKey,
        fullPublicKey: Buffer.from(address.subarray(1, 1 + 65)),
        compressedPublicKey: Buffer.from([yCoordParity + 2, ...address.subarray(2, 34)]),
        chainCode,
    };
}
